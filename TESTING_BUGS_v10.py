...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(backup_frame, text="Backup global temperatura:", font=("Segoe UI", 13)).grid(
        ...
        ctk.CTkLabel(backup_frame, text="Backup global CO2:", font=("Segoe UI", 13)).grid(
            row=1, column=0, sticky="w", pady=(6, 0)
        )
        ctk.CTkEntry(backup_frame, textvariable=self.backup_co2_path, font=("Segoe UI", 12)).grid(
            row=1,
            column=1,
            sticky="ew",
            padx=(4, 4),
        ...
        ctk.CTkButton(
            backup_frame,
            text="Destino…",
            command=self.pick_backup_co2,
            height=30,
            corner_radius=16,
        ).grid(row=1, column=2, padx=(4, 0), pady=(6, 0))

        backup_btns = ctk.CTkFrame(backup_frame, fg_color="transparent")
        backup_btns.grid(row=2, column=0, columnspan=3, sticky="w", pady=(8, 0))
        ctk.CTkButton(
            backup_btns,
            text="Borrar backup temperatura",
            command=self.clear_backup_temp,
            height=28,
            corner_radius=14,
        ).pack(side="left", padx=(0, 6))
        ctk.CTkButton(
            backup_btns,
            text="Borrar backup CO2",
            command=self.clear_backup_co2,
            height=28,
            corner_radius=14,
        ).pack(side="left")
    ...
    def _co2_backup_write_row(self, fermenter, ts, flow, current_ma, voltage, status):
        path = self.get_co2_backup_path()
        row = {
            "timestamp": ts.strftime("%Y-%m-%d %H:%M:%S"),
            "fermentador": fermenter,
    ...
    def _flow_take_sample(self, fermenter, ts=None):
        ...
        self._co2_backup_write_row(fermenter, ts, flow, current_ma, voltage, status)
    ...
    def open_flow_plot(self, fermenter):
        ...
        def load_flow_cache():
            ...
            data = self._read_recent_co2_backup(days=days_window, fermenter=fermenter)
            series = data.get(fermenter) if data else None
            if series and series["ts"]:
                flow_cache = list(zip(series["ts"], series["flow"]))
            else:
                flow_cache = []

        def combined_samples():
            samples = list(self.flow_samples.get(fermenter, []))
            if not flow_cache:
                return samples
            sample_ts = {ts for ts, *_ in samples}
            combined = [(ts, flow, None, None, "BACKUP") for ts, flow in flow_cache if ts not in sample_ts]
        ...
        def update_plot():
            ...
            nut_data = self._read_recent_backup(days=days_window, fermenter=fermenter)

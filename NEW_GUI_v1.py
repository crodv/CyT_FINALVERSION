...
"""
NEW_GUI.py
Versión liviana de la interfaz para TESTING_BUGS_v8.
Mantiene las mismas funcionalidades con una GUI más ligera y
carga asíncrona para los gráficos, evitando bloqueos al abrir ventanas.
"""
...
# ------------------------------------------------------------
# Lógica y constantes embebidas (antes importadas de TESTING_BUGS_v8)
# ------------------------------------------------------------
...
class LightFermenterPanel(ttk.Frame):
    def __init__(self, parent, app, name, hw: Hardware):
...
class RealTimePlotWindow:
    ...
    def request_data(self):
        ...
        def worker():
            data = self.app._read_recent_temp_csv(days=days_window, fermenter=self.fermenter)
    ...
    def _on_data(self, req_id, data):
        ...
        if not data:
            self.status.config(text="Sin datos recientes en CSV.")
        else:
            temps_all = []
            ...
            if self.current_window_hours is None:
                ...
                self.status.config(text="Fuente: CSV temperatura (tiempo real)")
            else:
                right = now()
                ...
                self.status.config(text=f"Fuente: CSV temperatura ({rango})")
...
class FlowPlotWindow:
    def __init__(self, app, fermenter):
        ...
        # CSV CO2 control
        csv_box = ttk.LabelFrame(self.top, text="CSV CO2")
        csv_box.pack(fill="x", padx=8, pady=(0, 6))
        csv_box.columnconfigure(1, weight=1)

        ttk.Label(csv_box, text="Carpeta:").grid(row=0, column=0, sticky="e", padx=(4, 2), pady=2)
        ttk.Entry(csv_box, textvariable=self.app.co2_csv_dir[self.fermenter]).grid(row=0, column=1, sticky="ew", pady=2)
        ttk.Button(csv_box, text="...", width=3, command=lambda: self.app.co2_pick_dir(self.fermenter)).grid(
            row=0, column=2, sticky="w", padx=2
        )

        ttk.Label(csv_box, text="Archivo:").grid(row=1, column=0, sticky="e", padx=(4, 2), pady=2)
        ttk.Entry(csv_box, textvariable=self.app.co2_csv_name[self.fermenter]).grid(row=1, column=1, sticky="ew", pady=2)

        btn_row = ttk.Frame(csv_box)
        btn_row.grid(row=2, column=0, columnspan=3, sticky="ew", pady=(4, 2))
        btn_row.columnconfigure((0, 1, 2, 3), weight=1)
        ttk.Button(btn_row, text="Inicio/Reanudar", command=lambda: self.app.co2_csv_start(self.fermenter)).grid(
            row=0, column=0, padx=2, sticky="ew"
        )
        ttk.Button(btn_row, text="Pausar/Detener", command=lambda: self.app.co2_csv_pause(self.fermenter)).grid(
            row=0, column=1, padx=2, sticky="ew"
        )
        ttk.Button(btn_row, text="Exportar CSV", command=lambda: self.app.co2_csv_export(self.fermenter)).grid(
            row=0, column=2, padx=2, sticky="ew"
        )
        ttk.Button(btn_row, text="Borrar/Reiniciar", command=lambda: self.app.co2_csv_restart(self.fermenter)).grid(
            row=0, column=3, padx=2, sticky="ew"
        )

        state_row = ttk.Frame(csv_box)
        state_row.grid(row=3, column=0, columnspan=3, sticky="w", pady=(2, 2))
        ttk.Label(state_row, text="Estado:").pack(side="left")
        self.led = Led(state_row)
        self.led.widget().pack(side="left", padx=6)
        self.app._co2_csv_register_led(self.fermenter, self.led)
        ...
        self.ax_nut.set_ylim(-0.1, 1.1)
        self.ax_nut.set_yticks([0, 1])
        self.ax_nut.set_xlabel("Hora")
        self.ax_nut.grid(True, alpha=0.2)
        self.ax_nut.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
    ...
    def close(self):
        ...
        self.app._co2_csv_unregister_led(self.fermenter, self.led)
        self.top.destroy()
    ...
    def refresh_plot(self):
        ...
        # nutrición desde cache
        if self.nut_cache:
            nut_times = []
            nut_values = []
            for ts, val in self.nut_cache:
                if left <= ts <= right:
                    nut_times.append(ts)
...
class App(tk.Tk):
    def __init__(self):
        ...
        for i in range(3):
            ...
            border.grid(row=1, column=i, padx=8, pady=(4, 10), sticky="nsew")
            border.grid_columnconfigure(0, weight=1)

            panel = LightFermenterPanel(border, self, f"F{i+1}", self.hw)
            panel.grid(row=0, column=0, sticky="nsew", padx=4, pady=4)
            self.ferms.append(panel)

        # Footer
        footer = ttk.Frame(main.inner)
        footer.grid(row=2, column=0, columnspan=3, sticky="ew", padx=12, pady=(4, 10))
    ...
    def _read_recent_temp_csv(self, days=10, fermenter=None):
        cutoff = now() - dt.timedelta(days=days)
        app_start = getattr(self, "app_start", None)
        if app_start:
            cutoff = max(cutoff, app_start)
        data = {}
        panels = {p.name: p for p in self.ferms}
        names = [fermenter] if fermenter else sorted(panels.keys())
        for name in names:
            panel = panels.get(name)
            if not panel:
                continue
            path = panel._csv_path()
                ...
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    for row in reader:
                        ...
                        if not ts or ts < cutoff:
                            continue
                        ferm = row.get("fermentador", name).strip() or name

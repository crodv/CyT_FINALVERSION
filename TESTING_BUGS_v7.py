...
SHUNT_OHMS = {
    ...
    "F2": float(os.environ.get("SHUNT_OHMS_F2", "148.0")),
    "F3": float(os.environ.get("SHUNT_OHMS_F3", "148.0")),
}
...
def _downsample_xy(xs, ys, max_points: int):
...
class FermenterPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        sp_entry = ctk.CTkEntry(
            sp_box,
            textvariable=self.sp_entry,
            width=80,
            justify="center",
            font=("Segoe UI", 18, "bold"),
        )
        sp_entry.pack(padx=4, pady=(4, 2))
        sp_entry.bind("<Return>", lambda _e: self.apply_sp())
        ctk.CTkButton(
            sp_box,
            text="Aplicar",
            command=self.apply_sp,
            fg_color="#22c55e",
            hover_color="#16a34a",
            corner_radius=12,
            height=26,
        ...
        band_entry = ctk.CTkEntry(
            band_box,
            textvariable=self.band_entry,
            width=80,
            justify="center",
            font=("Segoe UI", 18, "bold"),
        )
        band_entry.pack(padx=4, pady=(4, 2))
        band_entry.bind("<Return>", lambda _e: self.apply_band())
        ctk.CTkButton(
            band_box,
            text="Aplicar",
            command=self.apply_band,
            fg_color="#22c55e",
            hover_color="#16a34a",
            corner_radius=12,
            height=26,
        ...
        freq_entry = ctk.CTkEntry(
            nut_frame,
            textvariable=self.freq_entry,
            width=90,
            justify="center",
            font=("Segoe UI", 13),
        )
        freq_entry.grid(row=1, column=1, sticky="w", padx=(4, 0), pady=(6, 0))
        ...
        # CSV CO2
        co2_box = ctk.CTkFrame(self, fg_color="#020617", corner_radius=20, border_width=1, border_color="#1f2937")
        co2_box.grid(row=9, column=0, padx=16, pady=(0, 12), sticky="nsew")
        co2_box.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(co2_box, text="CSV CO2", font=("Segoe UI", 16, "bold")).grid(
            row=0,
            column=0,
            columnspan=3,
            sticky="w",
        ...
        ctk.CTkLabel(co2_box, text="Carpeta:", font=("Segoe UI", 13)).grid(
            row=1,
            column=0,
            sticky="e",
            padx=(12, 4),
        ...
        ctk.CTkEntry(
            co2_box,
            textvariable=self.app.co2_csv_dir[self.name],
            font=("Segoe UI", 12),
        ).grid(
            row=1,
            column=1,
        ...
        ctk.CTkButton(
            ...
            text="...",
            width=40,
            command=lambda: self.app.co2_pick_dir(self.name),
            corner_radius=12,
        ).grid(row=1, column=2, sticky="w", padx=(4, 12), pady=2)

        ctk.CTkLabel(co2_box, text="Archivo:", font=("Segoe UI", 13)).grid(
            row=2,
            column=0,
            sticky="e",
            padx=(12, 4),
        ...
        ctk.CTkEntry(
            co2_box,
            textvariable=self.app.co2_csv_name[self.name],
            font=("Segoe UI", 12),
        ).grid(
            row=2,
            column=1,
        ...
        btn_co2_row = ctk.CTkFrame(co2_box, fg_color="#020617")
        btn_co2_row.grid(row=3, column=0, columnspan=3, pady=(6, 4), padx=12, sticky="ew")
        btn_co2_row.grid_columnconfigure((0, 1, 2, 3), weight=1, uniform="co2csv")

        def _mk_co2_btn(text, cmd):
            return ctk.CTkButton(
                btn_co2_row,
                text=text,
                command=cmd,
        ...
        _mk_co2_btn("Inicio/Reanudar", lambda: self.app.co2_csv_start(self.name)).grid(
            row=0,
            column=0,
            padx=2,
            sticky="ew",
        )
        _mk_co2_btn("Pausar/Detener", lambda: self.app.co2_csv_pause(self.name)).grid(
            row=0,
            column=1,
            padx=2,
            sticky="ew",
        )
        _mk_co2_btn("Exportar CSV", lambda: self.app.co2_csv_export(self.name)).grid(
            row=0,
            column=2,
            padx=2,
            sticky="ew",
        )
        _mk_co2_btn("Borrar/Reiniciar", lambda: self.app.co2_csv_restart(self.name)).grid(
            row=0,
            column=3,
            padx=2,
            sticky="ew",
        ...
        state_co2 = ctk.CTkFrame(co2_box, fg_color="#020617")
        state_co2.grid(row=4, column=0, columnspan=3, sticky="ew", padx=12, pady=(4, 10))
        state_co2.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(state_co2, text="Estado:", font=("Segoe UI", 13)).grid(row=0, column=0, sticky="w")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        min_w = min(1400, avail_w)
        min_h = min(850, avail_h)
        self.minsize(min_w, min_h)

        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)

        # Scroll vertical para toda la pantalla (evita que se pierdan widgets en pantallas bajas)
        ...
        # Backup global
        backup_frame = ctk.CTkFrame(main, fg_color="transparent")
        ...
        self.ferms = []
        for i in range(3):
            panel = FermenterPanel(main, self, f"F{i+1}", self.hw, backup_path_getter=self.get_backup_path)
        ...
        self._tick_job = None        
        self._plot_windows = []
        self._plot_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="plot_io")
        self._closing = False
    ...
    def _read_recent_backup(self, days=10, fermenter=None):
        ...
        # Importante para la GUI: esta función se llama desde los gráficos.
        # Evitamos leer el CSV completo en cada refresh: leemos "desde el final"
        # y nos detenemos cuando encontramos muestras más antiguas que `cutoff`.
        data_rev = {}
        try:
            with open(path, "rb") as fb:
                ...
                while pos > 0 and not stop:
                    ...
                    for raw in reversed(complete):
                        ...
                        if header_line and line.strip() == header_line:
                            continue
                        try:
                            if fieldnames:
                        ...
                        ferm = (row.get("fermentador", "?") or "?").strip() or "?"
                        ...
                        series = data_rev.setdefault(ferm, {"rows": []})
                        series["rows"].append((ts, temp, sp, nut))
        except Exception:
            # Fallback (más lento, pero robusto) si el archivo tiene formato inesperado.
            data = {}
        ...
        data = {}
        for ferm, series in data_rev.items():
            rows = list(reversed(series.get("rows", [])))
            if not rows:
                continue
            out = data.setdefault(ferm, {"ts": [], "t": [], "sp": [], "nut": []})
            for ts, temp, sp, nut in rows:
                out["ts"].append(ts)
                out["t"].append(temp)
                out["sp"].append(sp)
                out["nut"].append(nut)
    ...
    def _plot_request_backup(self, top, days, fermenter, min_interval_sec: float):
        if not getattr(top, "winfo_exists", lambda: False)():
            return
        req = (float(days), fermenter or None)
        now_monotonic = time.monotonic()

        last_req = getattr(top, "_backup_req", None)
        last_started = getattr(top, "_backup_last_started", 0.0)
        future = getattr(top, "_backup_future", None)

        if future is not None and not future.done():
            return
        if last_req == req and (now_monotonic - last_started) < float(min_interval_sec):
            return

        top._backup_req = req
        top._backup_last_started = now_monotonic
        top._backup_future = self._plot_executor.submit(self._read_recent_backup, days, fermenter)

    def _plot_poll_backup(self, top):
        future = getattr(top, "_backup_future", None)
        if future is None:
            return getattr(top, "_backup_data", None)
        if not future.done():
            return getattr(top, "_backup_data", None)
        try:
            top._backup_data = future.result()
        except Exception:
            top._backup_data = {}
        top._backup_future = None
        return top._backup_data
    ...
    def open_flow_plot(self, fermenter):
        ...
        self._co2_csv_register_led(fermenter, led)
        ...
        fig, (ax_flow, ax_nut) = plt.subplots(
            2,
            1,
            sharex=True,
            figsize=(9, 7),
        ...
        line_nut = ax_nut.step([], [], where="post", color="red", alpha=0.8, label="Nutrición")[0]
        ...
        ax_nut.set_xlabel("Hora")
        ax_nut.grid(True, alpha=0.2)
        ax_nut.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
        ...
        def update_plot():
            ...
            if not windowed:
                ...
                line_nut.set_data([], [])
                return
            # Mantener la GUI fluida: limitar puntos a dibujar.
            n = len(windowed)
            stride = max(1, n // 2500)
            times = [windowed[i][0] for i in range(0, n, stride)]
            values = [windowed[i][1] for i in range(0, n, stride)]
            ...
            window_seconds = max(1.0, (right - left).total_seconds())
            days_window = max(1 / 24, window_seconds / 86400.0)
            # Leer backup en background (evita congelar el hilo Tk).
            self._plot_request_backup(top, days_window, fermenter=fermenter, min_interval_sec=10.0)
            nut_data = self._plot_poll_backup(top) or {}
            series = nut_data.get(fermenter)
            if series and series["ts"]:
                nut_times = []
                nut_values = []
                for ts, val in zip(series["ts"], series["nut"]):
                    if left <= ts <= right:
                        nut_times.append(ts)
                        nut_values.append(val)
                nut_times, nut_values = _compress_step(nut_times, nut_values)
                line_nut.set_data(nut_times, nut_values)
            else:
                line_nut.set_data([], [])
        ...
        def on_close_plot():
            ...
            self._co2_csv_unregister_led(fermenter, led)
            top.destroy()
    ...
    def open_realtime_plot(self, fermenter=None):
        ...
        def refresh():
            if not top.winfo_exists():
                return
            nonlocal current_window_hours
            days_window = 3650 if current_window_hours is None else (current_window_hours / 24.0)
            # Leer backup en background para no congelar Tk.
            self._plot_request_backup(top, days_window, fermenter=fermenter, min_interval_sec=5.0)
            data = self._plot_poll_backup(top) or {}
            ax_temp.clear()
            ax_nut.clear()

            future = getattr(top, "_backup_future", None)
            if not data and future is not None and not future.done():
                status.config(text="Cargando datos…")
            elif not data:
                ...
                for ferm, series in sorted(data.items()):
                    if not series["ts"]:
                        continue
                    ts_t, t_vals = _downsample_xy(series["ts"], series["t"], max_points=2500)
                    ts_sp, sp_vals = _downsample_xy(series["ts"], series["sp"], max_points=2500)
                    ax_temp.plot(ts_t, t_vals, label=f"{ferm} T")
                    ax_temp.plot(ts_sp, sp_vals, linestyle="--", label=f"{ferm} SP")
                    temps_all.extend(t_vals)
                    nut = series.get("nut", [])
                    if nut and any(nut):
                        nut_ts, nut_vals = _compress_step(series["ts"], nut)
                        ax_nut.step(nut_ts, nut_vals, where="post", color="red", alpha=0.8, label=f"{ferm} Nutrición")
                ...
                if current_window_hours is None:
                    ...
                    status.config(text="Fuente: backup global (tiempo real)")
                else:
                    right = now()
                    left = right - dt.timedelta(hours=current_window_hours)
                    ax_temp.set_xlim(left, right)
    ...
    def on_close(self):
        ...
        try:
            ...
            executor = getattr(self, "_plot_executor", None)
